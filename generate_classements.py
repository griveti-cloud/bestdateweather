...
def make_table_sun(entries, n, lang, country_info=None):
    headers = {
        ...
        'en': ('Rank','Destination','Sun/year',['Avg. rain'],'Score / Safety'),
        'es': ('Pos.','Destino','Sol/año',['Lluvia media'],'Punt. / Seg.'),
        'de': ('Rang','Ziel','Sonne/Jahr',['Ø Regen'],'Score / Sich.'),
    }
    h = headers['en' if lang == 'en-us' else lang]
    rows = []
    for i, entry in enumerate(entries[:n], 1):
...
def make_table_rain(entries, n, lang, country_info=None):
    headers = {
        ...
        'en': ('Rank','Destination','Avg. rain',['Sun/year'],'Score / Safety'),
        'es': ('Pos.','Destino','Lluvia media',['Sol/año'],'Punt. / Seg.'),
        'de': ('Rang','Ziel','Ø Regen',['Sonne/Jahr'],'Score / Sich.'),
    }
    h = headers['en' if lang == 'en-us' else lang]
    rows = []
    for i, entry in enumerate(entries[:n], 1):
