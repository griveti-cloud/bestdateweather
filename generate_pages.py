...
from scoring import profile_score as _profile_score
...
CORE_JS_VERSION = 65
APP_CSS_VERSION  = 52   # Bumper ici force le rechargement du cache CSS (app.css?v=N)
...
<a href="{flights_url}" target="_blank" rel="sponsored noopener" class="plan-link plan-link-flight">{_pb_flight_lbl}</a>
    ...
    monthly_links = ''.join(
        ...
        f'{MONTH_BTN_CLS.get(months[i]["classe"], "month-btn-mid")}" data-mi="{i}">'
    ...
    # Profils scoring — calcul des scores par profil pour les 12 mois
    _fiche_prof = {}
    for _mi, _m in enumerate(months):
        _fiche_prof[_mi] = {
            'sc': round(_profile_score(_m['tmax'], _m['rain_pct'], _m['sun_h'], _m.get('dew_point'), 'cool', _m.get('precip')) * 10, 1),
            'sw': round(_profile_score(_m['tmax'], _m['rain_pct'], _m['sun_h'], _m.get('dew_point'), 'warm', _m.get('precip')) * 10, 1),
            'sh': round(_profile_score(_m['tmax'], _m['rain_pct'], _m['sun_h'], _m.get('dew_point'), 'humid', _m.get('precip')) * 10, 1),
        }
    _fiche_prof_json = '{' + ','.join(f'{k}:{{sc:{v["sc"]},sw:{v["sw"]},sh:{v["sh"]}}}' for k, v in _fiche_prof.items()) + '}'

    # Labels profils selon langue
    _pbar_labels = {
        'fr': {'balanced': '🌤️ Standard', 'cool': '❄️ Frais', 'warm': '🔥 Chaud', 'humid': '💧 Sec'},
        'en': {'balanced': '🌤️ Balanced', 'cool': '❄️ Prefer cool', 'warm': '🔥 Prefer warm', 'humid': '💧 Humidity sensitive'},
        'en-us': {'balanced': '🌤️ Balanced', 'cool': '❄️ Prefer cool', 'warm': '🔥 Prefer warm', 'humid': '💧 Humidity sensitive'},
        'es': {'balanced': '🌤️ Estándar', 'cool': '❄️ Fresco', 'warm': '🔥 Cálido', 'humid': '💧 Sensible humedad'},
        'de': {'balanced': '🌤️ Standard', 'cool': '❄️ Kühl', 'warm': '🔥 Warm', 'humid': '💧 Feuchtigkeitssensibel'},
    }
    _pl = _pbar_labels.get(C['lang'], _pbar_labels['en'])
    _tip = {
        'fr': {'balanced': 'Confort optimal 22-28°C · Pluie pénalisée · Soleil valorisé',
               'cool': 'Confort optimal 15-20°C · Chaleur rapidement pénalisée',
               'warm': 'Confort optimal 27-32°C · Froid fortement pénalisé',
               'humid': 'Pénalité humidité renforcée (dew > 14°C)'},
        'en': {'balanced': 'Optimal comfort 22-28°C · Penalizes rain · Values sunshine',
               'cool': 'Optimal comfort 15-20°C · Heat penalized early',
               'warm': 'Optimal comfort 27-32°C · Cold strongly penalized',
               'humid': 'Strong humidity penalty (dew > 14°C)'},
    }
