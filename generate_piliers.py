    ...
    with open(ROOT / 'data/climate.csv', encoding='utf-8-sig') as f:
        for r in csv.DictReader(f):
            ...
            data[slug][m] = {
                ...
                'dew_point': float(r['dew_point_mean']) if r.get('dew_point_mean') else None,
            }
    return data
...
from lib.regions import (
    ...
    reg as _reg, REGION_LABELS as _REGION_LABELS, NON_EUROPE_SLUGS,
)
...
def get_rankings(climate, dests, month_idx):
    ...
    for slug, dest in dests.items():
        ...
        _pp = m.get('precip_mm')
        try: _pp = float(_pp) if _pp not in (None,'','None') else None
        except: _pp = None
        hike = compute_hiking_score(m['tmax'], m['rain_pct'], m['sun_h'], _pp)
        entries.append({
            'slug_fr': slug,
...
def get_pool_entries(climate, dests, month_idx, pool_size=80, ski_boost=80):
    ...
    for slug, dest in dests.items():
        ...
        m = climate[slug][month_idx]
        ski = compute_ski_score(m['tmax'], m['rain_pct'], m['sun_h'])
        _pp2 = m.get('precip_mm')
        try: _pp2 = float(_pp2) if _pp2 not in (None,'','None') else None
        except: _pp2 = None
        hike = compute_hiking_score(m['tmax'], m['rain_pct'], m['sun_h'], _pp2)
        all_entries.append({
            'slug_fr': slug,
            'slug_en': dest.get('slug_en', slug),
            'nom_bare': dest.get('nom_bare', slug),
    ...
    # --- Mountain boost: top destinations par max(ski, hike) pour couvrir rando ---
    ski_candidates = [e for e in all_entries if e['is_mountain'] and e['slug_fr'] not in general_slugs and e['tmax'] <= 30]
    ski_candidates.sort(key=lambda x: (-max(x['ski_score'], x.get('hiking_score', 0)), x['nom_bare']))
...
def generate_page(mi, lang, dests, climate, country_info=None):
    ...
    pool_json = json.dumps([{
        ...
        'hk': round(p.get('hiking_score', 0), 1),
        'mtn': round(max(p['ski_score'], p.get('hiking_score', 0)), 1),
        ...
        'dew': round(p['dew_point'], 1) if p.get('dew_point') is not None else None,
        'sc': round(profile_score(p['tmax'], p['rain_pct'], p['sun_h'], p.get('dew_point'), 'cool', p.get('precip_mm')) * 10, 1),
        'sw': round(profile_score(p['tmax'], p['rain_pct'], p['sun_h'], p.get('dew_point'), 'warm', p.get('precip_mm')) * 10, 1),
        'sh': round(profile_score(p['tmax'], p['rain_pct'], p['sun_h'], p.get('dew_point'), 'humid', p.get('precip_mm')) * 10, 1),
    ...
    _secu_items = f'<div class="fc-item active threshold-active" data-rl="4" onclick="event.stopPropagation();setSecu(4)">{_secu_all_lbl}</div>' + ''.join(f'<div class="fc-item" data-rl="{r}" onclick="event.stopPropagation();setSecu({r})">≤ {sl.get(str(r),str(r))}</div>' for r in range(1,4))
    _budget_items = f'<div class="fc-item active threshold-active" data-bi="5" onclick="event.stopPropagation();setBudget(5)">{_budget_all_lbl}</div>' + ''.join(f'<div class="fc-item" data-bi="{b}" onclick="event.stopPropagation();setBudget({b})">≤ {bl.get(str(b),str(b))}</div>' for b in range(1,5))
    ...
    _profile_items = _build_profile_items(lang)
    _profile_chip_lbl = _build_profile_label(lang, 'balanced')
    filter_chips_html = (
        ...
        f'</div></div></div>'
        f'<div class="fchip" id="fc-profile" data-fc="profile" onclick="toggleFC(this.dataset.fc,event)">'
        f'<span id="fc-profile-lbl">{_profile_chip_lbl}</span>'
        f'<span class="fchip-arrow">▾</span>'
        f'<div class="fc-drop" id="fcd-profile">{_profile_items}</div></div>'
        f'<div class="fchip" id="fc-mtn-sub" data-fc="mtn-sub" onclick="toggleFC(this.dataset.fc,event)" style="display:none">'
        f'<span id="fc-mtn-sub-lbl">{pil.get("tab_mountain_sub_max","Max")}</span>'
        f'<span class="fchip-arrow">▾</span>'
        f'<div class="fc-drop" id="fcd-mtn-sub">'
        f'<div class="fc-item active" data-mtn-sub="max" onclick="event.stopPropagation();setMtnSub(\'max\')">{pil.get("tab_mountain_sub_max","Max")}</div>'
        f'<div class="fc-item" data-mtn-sub="ski" onclick="event.stopPropagation();setMtnSub(\'ski\')">{pil.get("tab_mountain_sub_ski","⛷️ Ski")}</div>'
        f'<div class="fc-item" data-mtn-sub="rando" onclick="event.stopPropagation();setMtnSub(\'rando\')">{pil.get("tab_mountain_sub_rando","🥾 Rando")}</div>'
        f'</div></div>'
    )

    # Barre de profils : intégrée dans filter-bar comme 6e dropdown (fc-profile).
    # Variable conservée vide pour compat avec les templates qui l'injectent.
    _profile_bar = ''
    ...
    rank_js = (
        ...
        'if(!tb)return;'+
        'var mtnKey=CUR_MTN_SUB==="ski"?"k":(CUR_MTN_SUB==="rando"?"hk":"mtn");'+
        'var key=mode==="beach"?"b":mode==="ski"?mtnKey:mode==="nomad"?"nd":"s";'+
        'var useProf=CUR_PROF!=="balanced"&&mode==="meteo";'+
        'if(useProf){POOL.forEach(function(d){d._ps=_profScore(d,CUR_PROF);});}'+
        'var list=POOL.filter(function(d){var rOk=CUR_REG==="all"||(d.reg===CUR_REG);var mOk=mode==="beach"?(d.b!=null&&d.b>=3.5):mode==="ski"?(d.m===1&&(d[mtnKey]||0)>=4&&(CUR_MTN_SUB==="ski"?d.tmax<=25:true)):mode==="nomad"?(d.nd!=null):(d.m!==1);var rlOk=mode==="nomad"?true:(d.rl||1)<=CUR_RL;var biOk=mode==="nomad"?true:(d.bi||3)<=CUR_BI;return rOk&&mOk&&rlOk&&biOk;});'+
        ...
        '+"<td style=\'text-align:center\'>"+({\'1\':\'🟢\',\'2\':\'🟡\',\'3\':\'🟠\',\'4\':\'🔴\'}[String(d.rl||1)]||\'🟢\')+"</td>"'+
        '+"<td style=\'text-align:center\'>"+({\'1\':\'💚\',\'2\':\'💛\',\'3\':\'🟡\',\'4\':\'🟠\',\'5\':\'🔴\'}[String(d.bi||3)]||\'🟡\')+"</td>"'+
        '+"</tr>";'+
        '});'+
        'tb.innerHTML=html;'+
        ...
        '}}'+
        '}'+
        'function setMode(m){CUR_MODE=m;if(m==="nomad"&&typeof ANNUAL_URL!=="undefined"&&ANNUAL_URL){var reg=CUR_REG&&CUR_REG!=="all"?"&reg="+CUR_REG:"";location.href=ANNUAL_URL+"?mode=nomad"+reg;return;}document.querySelectorAll(".fc-item[data-mode]").forEach(function(b){b.classList.toggle("active",b.dataset.mode===m);});var lm=document.querySelector(".fc-item.active[data-mode]");if(lm)document.getElementById("fc-type-lbl").textContent=lm.textContent.trim();document.getElementById("fc-type").classList.toggle("has-filter",m!=="meteo");closeFC();var isNomad=m==="nomad";var _chips=["fc-secu","fc-budget","fc-period","fc-profile"];_chips.forEach(function(id){var el=document.getElementById(id);if(el){el.style.display=isNomad?"none":"";el.style.pointerEvents=isNomad?"none":"";}});var _nom=document.getElementById("rt-methodo-nomad");if(_nom)_nom.style.display=isNomad?"block":"none";var _fcp=document.getElementById("fc-profile");if(_fcp)_fcp.style.display=m==="meteo"?"":"none";var _fcms=document.getElementById("fc-mtn-sub");if(_fcms)_fcms.style.display=m==="ski"?"":"none";render(m);}'+
        ...
        'function setSecu(rl){CUR_RL=rl;document.querySelectorAll(".fc-item[data-rl]").forEach(function(b){b.classList.toggle("threshold-active",parseInt(b.dataset.rl)<=rl);b.classList.toggle("active",parseInt(b.dataset.rl)===rl);});var ls=document.querySelector(".fc-item[data-rl=\'"+rl+"\' ]");var lbl=ls?ls.textContent.trim().replace(/^≤\s*/,""):"";document.getElementById("fc-secu-lbl").textContent=rl<4?("🛡 \u2264 "+lbl):"'+ fp_secu +'";document.getElementById("fc-secu").classList.toggle("has-filter",rl<4);closeFC();var am=document.querySelector(".fc-item.active[data-mode]");render(am?am.dataset.mode:"meteo");}'+
        'function setBudget(bi){CUR_BI=bi;document.querySelectorAll(".fc-item[data-bi]").forEach(function(b){b.classList.toggle("threshold-active",parseInt(b.dataset.bi)<=bi);b.classList.toggle("active",parseInt(b.dataset.bi)===bi);});var lb=document.querySelector(".fc-item[data-bi=\'"+bi+"\' ]");var lbl=lb?lb.textContent.trim().replace(/^≤\s*/,""):"";document.getElementById("fc-budget-lbl").textContent=bi<5?("\u2264 "+lbl):"'+ fp_budget +'";document.getElementById("fc-budget").classList.toggle("has-filter",bi<5);closeFC();var am=document.querySelector(".fc-item.active[data-mode]");render(am?am.dataset.mode:"meteo");}'+
        'window._fcClosedAt=0;function closeFC(){window._fcClosedAt=Date.now();document.querySelectorAll(".fchip.open").forEach(function(c){c.classList.remove("open");var id=c.id.replace("fc-","");var d=document.getElementById("fcd-"+id);if(d)d.style.display="none";});}window.addEventListener("scroll",function(){if(document.querySelector(".fchip.open"))closeFC();},{passive:true});'+
        'function toggleFC(id,ev){if(ev)ev.stopPropagation();if(Date.now()-window._fcClosedAt<50)return;var chip=document.getElementById("fc-"+id);var drop=document.getElementById("fcd-"+id);var wasOpen=chip.classList.contains("open");closeFC();if(!wasOpen){chip.classList.add("open");var r=chip.getBoundingClientRect();var l=Math.max(8,Math.min(r.left,window.innerWidth-168));drop.style.cssText="display:block;position:fixed;top:"+(r.bottom+4)+"px;left:"+l+"px;z-index:9999;min-width:160px;max-width:220px;background:white;border:1.5px solid #e8e0d0;border-radius:12px;box-shadow:0 8px 24px rgba(26,31,46,.18);padding:6px";}}'+

        'var _p=new URLSearchParams(location.search);var _initMode=_p.get("mode")||"meteo";var _initReg=_p.get("reg")||"all";var _initRL=parseInt(_p.get("rl")||"4");var _initBI=parseInt(_p.get("bi")||"5");CUR_REG=_initReg;CUR_RL=_initRL;CUR_BI=_initBI;if(_initReg!=="all")setReg(_initReg);if(_initMode!=="meteo")setMode(_initMode);if(_initRL<4)setSecu(_initRL);if(_initBI<5)setBudget(_initBI);render(_initMode);fetch("/api/advisories").then(function(r){return r.ok?r.json():null}).catch(function(){return null}).then(function(adv){if(!adv||adv.error)return;var updated=adv._updated,src=adv._source;for(var i=0;i<POOL.length;i++){var cc=(POOL[i].f||"").toUpperCase();if(cc&&adv[cc]!=null)POOL[i].rl=Math.max(POOL[i].rl||1,adv[cc]);}var am=document.querySelector(".fc-item.active[data-mode]");render(am?am.dataset.mode:"meteo");var el=document.getElementById("secu-source");if(el&&updated)el.innerHTML="⚠ À vérifier · Sources : MAE (FR) + "+src+" · niveau le plus prudent · "+updated;});'+
        ...
        # JS setProfile — recalcul côté client
        'var CUR_PROF="balanced";var CUR_MODE="meteo";var CUR_MTN_SUB="max";'+
        'function _tIdeal(t,prof){'+
        'if(prof==="cool"){'+
        'if(t<=3)return 0;if(t<=10)return(t-3)/7*.3;if(t<=18)return .3+(t-10)/8*.6;'+
        'if(t<=22)return .9-(t-18)/4*.2;if(t<=27)return .7-(t-22)/5*.4;'+
        'if(t<=32)return .3-(t-27)/5*.25;return 0;}'+
        'if(prof==="warm"){'+
        'if(t<=8)return 0;if(t<=18)return(t-8)/10*.2;if(t<=24)return .2+(t-18)/6*.4;'+
        'if(t<=32)return .6+(t-24)/8*.4;if(t<=36)return 1-(t-32)/4*.3;'+
        'if(t<=40)return .7-(t-36)/4*.5;return 0;}'+
        'if(t<=5)return 0;if(t<=14)return(t-5)/9*.3;if(t<=22)return .3+(t-14)/8*.5;'+
        'if(t<=28)return .8+(t-22)/6*.2;if(t<=31)return 1-(t-28)/3*.1;'+
        'if(t<=34)return .9-(t-31)/3*.3;if(t<=37)return .6-(t-34)/3*.35;'+
        'if(t<=40)return .25-(t-37)/3*.2;return 0;}'+
        'function _dewPen(tmax,dew,prof){'+
        'var thresh=prof==="humid"?22:26;var dewMin=prof==="humid"?14:16;var maxPen=prof==="humid"?.35:.20;'+
        'if(tmax<thresh||dew==null||dew<dewMin)return 0;'+
        'var hf=Math.min(1,(tmax-thresh)/12);var df=Math.min(1,(dew-dewMin)/10);'+
        'return maxPen*hf*df;}'+
        'function _profScore(d,prof){'+
        'var t=_tIdeal(d.tmax,prof);'+
        'var r=Math.max(0,1-d.r/100);'+
        'var s=Math.min(1,d.sun/15);'+
        'var raw=.40*t+.35*r+.25*s;'+
        'raw-=_dewPen(d.tmax,d.dew,prof);'+
        'return Math.max(0,Math.min(10,raw*10));}'+
        'function _sc(v){return v>=8.6?"#1a7a4a":v>=7.6?"#2d9e60":v>=6.3?"#84cc16":v>=5?"#f59e0b":v>=3.5?"#f97316":"#ef4444";}'+
        'function setProfile(prof){'+
        'CUR_PROF=prof;'+
        'document.querySelectorAll(".fc-item[data-prof]").forEach(function(b){b.classList.toggle("active",b.dataset.prof===prof);});'+
        'var ap=document.querySelector(".fc-item.active[data-prof]");'+
        'if(ap){var lbl=ap.querySelector(".fc-item-lbl");var el=document.getElementById("fc-profile-lbl");if(el&&lbl)el.textContent=lbl.textContent.trim();}'+
        'var fcp=document.getElementById("fc-profile");if(fcp)fcp.classList.toggle("has-filter",prof!=="balanced");'+
        'closeFC();'+
        'var am=document.querySelector(".fc-item.active[data-mode]");'+
        'render(am?am.dataset.mode:"meteo");'+
        '}'+
        'function setMtnSub(sub){CUR_MTN_SUB=sub;document.querySelectorAll(".fc-item[data-mtn-sub]").forEach(function(b){b.classList.toggle("active",b.dataset.mtnSub===sub);});var lm=document.querySelector(".fc-item.active[data-mtn-sub]");if(lm){var el=document.getElementById("fc-mtn-sub-lbl");if(el)el.textContent=lm.textContent.trim();}var fcms=document.getElementById("fc-mtn-sub");if(fcms)fcms.classList.toggle("has-filter",sub!=="max");closeFC();var am=document.querySelector(".fc-item.active[data-mode]");render(am?am.dataset.mode:"meteo");}'+
        'window.setProfile=setProfile;window.setMtnSub=setMtnSub;'+
...
def get_annual_pool(climate, dests, pool_size=80, ski_boost=80):
    ...
    for slug, dest in dests.items():
        ...
        ski_vals = [compute_ski_score(float(months[m]['tmax']),
                    ...
                    for m in range(1,13)]
        # Use BEST ski score (pic annuel) — plus cohérent avec fiches destination
        # et agnostique hémisphère (Bariloche pic en juin-août vs Alpes en jan-mar)
        ski_winter = max(ski_vals) if ski_vals else 0
        # hiking score : même logique, pic annuel
        hike_vals = []
        for m in range(1,13):
            pp = months[m].get('precip_mm')
            try: pp = float(pp) if pp not in (None,'','None') else None
            except: pp = None
            hike_vals.append(compute_hiking_score(
                float(months[m]['tmax']),
                float(months[m]['rain_pct']),
                float(months[m]['sun_h']),
                pp))
        hike_best = max(hike_vals) if hike_vals else 0
    ...
    # Mountain boost: tri par max(ski, hike) pour couvrir ski ET rando
    ski_candidates = [e for e in all_entries if e['is_mountain'] and e['slug_fr'] not in general_slugs and e['tmax'] <= 30]
    ski_candidates.sort(key=lambda x: (-max(x['ski_score'], x.get('hiking_score', 0)), x['nom_bare']))
    ski_injected = ski_candidates[:ski_boost]
...
def get_annual_rankings(climate, dests, pool_size=80):
    ...
    for slug, dest in dests.items():
        ...
        raw.append({
            ...
            'rain_pct':  float(m_data['rain_pct']),
            'sun_h':     float(m_data['sun_h']),
            'dew_point_ann': float(m_data['dew_point']) if m_data.get('dew_point') else None,
            'dest':      dest,
        })
...
def _build_profile_bar(lang):
    """DEPRECATED: ancien format barre séparée. Conservé pour compat.
    Utiliser _build_profile_items() + _build_profile_label() à la place."""
    return ''
...
def _build_profile_items(lang):
    """HTML des 4 items du dropdown Profil (label + description)."""
    T = _profile_translations()
    pl = T.get(lang, T['en'])
    def _item(prof, active=False):
        cls = 'fc-item fc-item-profile' + (' active' if active else '')
        return (
            f'<div class="{cls}" data-prof="{prof}" '
            f'onclick="event.stopPropagation();setProfile(\'{prof}\')">'
            f'<span class="fc-item-lbl">{pl[prof]}</span>'
            f'<span class="fc-item-sub">{pl["sub_"+prof]}</span>'
            f'</div>'
        )
    return _item('balanced', active=True) + _item('cool') + _item('warm') + _item('humid')


def _build_profile_label(lang, prof='balanced'):
    """Label court pour le chip profil dans la filter-bar."""
    T = _profile_translations()
    pl = T.get(lang, T['en'])
    return pl.get(prof, pl['balanced'])
...
def generate_annual_page(lang, dests, climate, country_info=None):
    ...
    pool_json = json.dumps([{
        ...
        'hk':  round(e.get('hiking_score', 0), 1),
        'mtn': round(max(e['ski_score'], e.get('hiking_score', 0)), 1),
        ...
        'dew': round(e.get('dew_point_ann', 0), 1) if e.get('dew_point_ann') is not None else None,
        'sc': round(profile_score(e['tmax'], e['rain_pct'], e['sun_h'], e.get('dew_point_ann'), 'cool') * 10, 1),
        'sw': round(profile_score(e['tmax'], e['rain_pct'], e['sun_h'], e.get('dew_point_ann'), 'warm') * 10, 1),
        'sh': round(profile_score(e['tmax'], e['rain_pct'], e['sun_h'], e.get('dew_point_ann'), 'humid') * 10, 1),
    ...
    _secu_items_ann = f'<div class="fc-item active threshold-active" data-rl="4" onclick="event.stopPropagation();setSecu(4)">{_secu_all_lbl_ann}</div>' + ''.join(f'<div class="fc-item" data-rl="{r}" onclick="event.stopPropagation();setSecu({r})">≤ {_sl.get(str(r),str(r))}</div>' for r in range(1,4))
    _budget_items_ann = f'<div class="fc-item active threshold-active" data-bi="5" onclick="event.stopPropagation();setBudget(5)">{_budget_all_lbl_ann}</div>' + ''.join(f'<div class="fc-item" data-bi="{b}" onclick="event.stopPropagation();setBudget({b})">≤ {_bl.get(str(b),str(b))}</div>' for b in range(1,5))
    ...
    _annual_profile_bar = ''  # DEPRECATED: profil intégré dans filter-bar comme 6e dropdown
    _profile_items_ann = _build_profile_items(lang)
    _profile_chip_lbl_ann = _build_profile_label(lang, 'balanced')
    filter_chips_html = (
        f'<div class="fchip has-filter" id="fc-period" data-fc="period" onclick="toggleFC(this.dataset.fc,event)">'
        ...
        f'<div class="fchip" id="fc-profile" data-fc="profile" onclick="toggleFC(this.dataset.fc,event)">'
        f'<span id="fc-profile-lbl">{_profile_chip_lbl_ann}</span>'
        f'<span class="fchip-arrow">▾</span>'
        f'<div class="fc-drop" id="fcd-profile">{_profile_items_ann}</div></div>'
        f'<div class="fchip" id="fc-mtn-sub" data-fc="mtn-sub" onclick="toggleFC(this.dataset.fc,event)" style="display:none">'
        f'<span id="fc-mtn-sub-lbl">{pil.get("tab_mountain_sub_max","Max")}</span>'
        f'<span class="fchip-arrow">▾</span>'
        f'<div class="fc-drop" id="fcd-mtn-sub">'
    ...
    rank_js = (
        ...
        'function setProfile(prof){CUR_PROF=prof;document.querySelectorAll(".fc-item[data-prof]").forEach(function(b){b.classList.toggle("active",b.dataset.prof===prof);});var ap=document.querySelector(".fc-item.active[data-prof]");if(ap){var lbl=ap.querySelector(".fc-item-lbl");var el=document.getElementById("fc-profile-lbl");if(el&&lbl)el.textContent=lbl.textContent.trim();}var fcp=document.getElementById("fc-profile");if(fcp)fcp.classList.toggle("has-filter",prof!=="balanced");closeFC();var am=document.querySelector(".fc-item.active[data-mode]");render(am?am.dataset.mode:"meteo");}'+
        'function setMtnSub(sub){CUR_MTN_SUB=sub;document.querySelectorAll(".fc-item[data-mtn-sub]").forEach(function(b){b.classList.toggle("active",b.dataset.mtnSub===sub);});var lm=document.querySelector(".fc-item.active[data-mtn-sub]");if(lm){var el=document.getElementById("fc-mtn-sub-lbl");if(el)el.textContent=lm.textContent.trim();}var fcms=document.getElementById("fc-mtn-sub");if(fcms)fcms.classList.toggle("has-filter",sub!=="max");closeFC();var am=document.querySelector(".fc-item.active[data-mode]");render(am?am.dataset.mode:"meteo");}'+
        'window.setProfile=setProfile;window.setMtnSub=setMtnSub;'+
        ...
        'var _p=new URLSearchParams(location.search);var _initMode=_p.get("mode")||"meteo";var _initReg=_p.get("reg")||"all";var _initRL=parseInt(_p.get("rl")||"4");var _initBI=parseInt(_p.get("bi")||"5");CUR_REG=_initReg;CUR_RL=_initRL;CUR_BI=_initBI;if(_initReg!=="all")setReg(_initReg);if(_initMode!=="meteo")setMode(_initMode);if(_initRL<4)setSecu(_initRL);if(_initBI<5)setBudget(_initBI);render(_initMode);fetch("/api/advisories").then(function(r){return r.ok?r.json():null}).catch(function(){return null}).then(function(adv){if(!adv||adv.error)return;var updated=adv._updated,src=adv._source;for(var i=0;i<POOL.length;i++){var cc=(POOL[i].f||"").toUpperCase();if(cc&&adv[cc]!=null)POOL[i].rl=Math.max(POOL[i].rl||1,adv[cc]);}var am=document.querySelector(".fc-item.active[data-mode]");render(am?am.dataset.mode:"meteo");var el=document.getElementById("secu-source");if(el&&updated)el.innerHTML="⚠ À vérifier · Sources : MAE (FR) + "+src+" · niveau le plus prudent · "+updated;});document.querySelectorAll(".month-nav a").forEach(function(a){a.addEventListener("click",function(ev){var am=document.querySelector(".mode-tab.active");var ar=document.querySelector(".reg-tab.active");var m=am?am.dataset.mode:"meteo";var r=ar?ar.dataset.reg:"all";var rl=CUR_RL;var bi=CUR_BI;if(m==="meteo"&&r==="all"&&rl===4&&bi===5)return;ev.preventDefault();var url=this.href.split("?")[0];var q=[];if(m!=="meteo")q.push("mode="+m);if(r!=="all")q.push("reg="+r);if(rl<4)q.push("rl="+rl);if(bi<5)q.push("bi="+bi);location.href=url+(q.length?"?"+q.join("&"):"");});});'+
        'window.goMonth=function(el,ev){var am=document.querySelector(".fc-item.active[data-mode]");var ar=document.querySelector(".fc-item.active[data-reg]");var m=am?am.dataset.mode:"meteo";var r=ar?ar.dataset.reg:"all";var rl=CUR_RL;var bi=CUR_BI;if(m==="meteo"&&r==="all"&&rl===4&&bi===5)return true;ev.preventDefault();var url=el.href.split("?")[0];var q=[];if(m!=="meteo")q.push("mode="+m);if(r!=="all")q.push("reg="+r);if(rl<4)q.push("rl="+rl);if(bi<5)q.push("bi="+bi);location.href=url+(q.length?"?"+q.join("&"):"");return false;};'+
        'window.toggleFC=toggleFC;window.setMode=setMode;window.setReg=setReg;window.setSecu=setSecu;window.setBudget=setBudget;'+'})();</script>'
    )
