...
function setConfBadge(diffDays) {  }
...
if (code >= 51) code = (code >= 45 && code <= 48) ? 45 : 2;
}
if (code === 0) return '☀️';
if (code <= 2) return '⛅';
if (code === 3) return '☁️';
...
monthly[it.i].ficheScore = Math.round((lo + norm * (hi - lo)) * 10);
});
});
}
...
if (rh <= 65) return 0;
var tempFactor = Math.min(1, (avgTemp - 24) / 14);
var rhFactor = Math.min(1, (rh - 65) / 30);
return Math.round(20 * tempFactor * rhFactor);
}
...
var sunD = data.daily.sunshine_duration || [];
for (var i = 0; i < times.length; i++) {
var mo = parseInt(times[i].split('-')[1], 10) - 1;
...
var sun = d.sunHrs || 0;
var sTempSki;
if (tmax > 10) sTempSki = 0;
else if (tmax > 5) sTempSki = Math.max(0, 50 - (tmax - 5) * 10);
else if (tmax >= -2) sTempSki = 90 + (2 - Math.abs(tmax)) * 2;
else if (tmax >= -12) sTempSki = 90 - (Math.abs(tmax) - 2) * 3;
else sTempSki = Math.max(30, 90 - (Math.abs(tmax) - 2) * 3);
...
else if (tmax < 22) sTempPlage = (tmax - 18) / 4 * 20;
else if (tmax < 26) sTempPlage = 20 + (tmax - 22) / 4 * 60;
else if (tmax <= 35) sTempPlage = 80 + (tmax - 26) / 9 * 20;
else if (tmax <= 40) sTempPlage = 100 - (tmax - 35) / 5 * 30;
...
annAcTimer=setTimeout(function(){fetch('https:
};
