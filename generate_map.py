...
.pi-bars{{display:flex;gap:1px;align-items:flex-end;height:36px;margin-bottom:8px}}
...
var worldLayer=L.geoJSON(WORLD,{{style:{{fillColor:'#1e2640',fillOpacity:0.85,color:'#3a4a6a',weight:0.6}}}}).addTo(map);
map.on('zoomend',function(){{
  var z=map.getZoom();
  if(z>=6){{if(map.hasLayer(worldLayer))map.removeLayer(worldLayer);}}
...
function render(){{
  ...
  (window.MAP_DATA||[]).forEach(function(d){{
    ...
    mk.on('click',function(){{
      var bars=(d[8]||[]).map(function(ms,i){{var h=ms?Math.round(((ms[0]||0)/10)*100):0;var c=ms?scoreColor(ms[0]):'#333';var lbl=MS[i]?MS[i][0]:'';return '<div style="display:flex;flex-direction:column;align-items:center;flex:1;justify-content:flex-end"><div class="pi-bar'+(i===CUR_M?' cur':'')+'\" style="height:'+h+'%;background:'+c+';width:100%"></div><span style="font-size:6px;color:#64748b;margin-top:1px;line-height:1">'+lbl+'</span></div>';}}).join('');
